...
from pytorch_lightning.accelerators.npu import NPUAccelerator
...
class _AcceleratorConnector:
    def __init__(
        ...
        _register_external_accelerators_and_strategies() #register hpu
        self._registered_strategies = StrategyRegistry.available_strategies()
        self._accelerator_types = AcceleratorRegistry.available_accelerators()
        print(f"当前可用strategy:{self._registered_strategies}")
        print(f"当前可用accelerator:{self._accelerator_types}") #TODO
        ...
        print(f"self._accelerator_flag:{self._accelerator_flag}")
        ...
        self.cluster_environment: ClusterEnvironment = self._choose_and_init_cluster_environment() #Lightning
    ...
    def _choose_strategy(self) -> Union[Strategy, str]:
        ...
        if len(self._parallel_devices) <= 1:
            if isinstance(self._accelerator_flag, (CUDAAccelerator, MPSAccelerator)) or (
                isinstance(self._accelerator_flag, str) and self._accelerator_flag in ("cuda", "gpu", "npu", "mps")
            ):
